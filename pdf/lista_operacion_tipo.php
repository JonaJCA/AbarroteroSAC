...
class MiPDF extends FPDF {
	...
	$mipdf -> Ln (2);
	$mipdf -> Cell('mm',10,utf8_decode("LISTA DE OPERACIONES POR TIPO"),0,0,'C');
	$mipdf -> Ln (10);

	$mipdf -> Setfont('Arial','B',10);
	$mipdf -> Ln (2);
	$mipdf -> Cell('mm',10,utf8_decode("TIPO DE OPERACIÓN: $tipo_ope."),0,0,'L');
	$mipdf -> Ln (10);
    ...
    $sql="SELECT CAST(@s:=@s+1 AS UNSIGNED) AS orden, operaciones.id_operacion, descripcion_ope, tipo_ope, categoria_ope FROM operaciones, (SELECT @s:=0) AS s WHERE tipo_ope='$tipo_ope' ORDER BY orden, descripcion_ope ASC";
