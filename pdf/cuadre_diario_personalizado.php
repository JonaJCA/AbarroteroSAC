...
class MiPDF extends FPDF {
	...
	$query="SELECT (CONCAT(nombre_suc,' - ',direccion_suc,' (',nombre_depa,', ',nombre_provi,', ',nombre_dist,')')) AS SUCURSAL FROM administrador, sucursales, departamentos, provincias, distritos, movimientos WHERE movimientos.id_sucursal='$id_sucursal' AND administrador.id_sucursal=sucursales.id_sucursal AND sucursales.id_departamento=departamentos.id_departamento AND sucursales.id_provincia=provincias.id_provincia AND sucursales.id_distrito=distritos.id_distrito AND provincias.id_departamento=departamentos.id_departamento AND distritos.id_provincia=provincias.id_provincia AND movimientos.id_sucursal=sucursales.id_sucursal;";
		...
		$mipdf -> addPage();
			...
			$mipdf -> Ln (2);
			$mipdf -> Cell('mm',10,utf8_decode("CUADRE DE CAJA PERSONALIZADO: $TOPE"),0,0,'C');
			$mipdf -> Ln (10);

			$mipdf -> Setfont('Arial','B',10);
			$mipdf -> Ln (2);
			$mipdf -> Cell('mm',10,utf8_decode("PERIODO: $TOPE."),0,0,'L');
			$mipdf -> Ln (2);
...
$consulta_principal="SELECT movimientos.id_sucursal, DATE_FORMAT(movimientos.fecha_movi, '%d/%m/%Y') AS FECHA, operaciones.tipo_ope, operaciones.descripcion_ope AS OPERACION, movimientos.descripcion_movi AS DESCRIPCION, CONCAT('S/', FORMAT((CASE operaciones.tipo_ope WHEN 'INGRESOS' THEN (monto_movi) ELSE '0' END), 2)) AS INGRESOS, CONCAT('S/', FORMAT((CASE operaciones.tipo_ope WHEN 'EGRESOS' THEN (monto_movi) ELSE (monto_sub_movi) END), 2)) AS EGRESOS, CASE operaciones.tipo_ope WHEN 'INGRESOS' THEN 'INGRESOS' WHEN 'EGRESOS' THEN (CASE movimientos.estado_movi WHEN 'POR CANJEAR' THEN 'VALES' ELSE 'EGRESOS' END) END AS TIPO FROM movimientos, operaciones, sucursales, documentos WHERE movimientos.id_documento=documentos.id_documento AND movimientos.id_operacion=operaciones.id_operacion AND movimientos.id_sucursal=sucursales.id_sucursal AND (movimientos.estado_movi='REALIZADO' OR movimientos.estado_movi='LIQUIDADO' OR movimientos.estado_movi='COBRO' OR movimientos.estado_movi='POR CANJEAR') AND sucursales.id_sucursal='$id_sucursal' AND (movimientos.fecha_movi ='$periodo') ORDER BY operaciones.tipo_ope DESC, TIPO ASC, movimientos.fecha_movi DESC;";
