...
class MiPDF extends FPDF {
	...
	$mipdf -> Ln (2);
	$mipdf -> Cell('mm',10,utf8_decode("LISTA DE EGRESOS POR ESTADO DE OPERACIÓN"),0,0,'C');
	$mipdf -> Ln (10);

	$mipdf -> Setfont('Arial','B',10);
	$mipdf -> Ln (2);
	$mipdf -> Cell('mm',10,utf8_decode("ESTADO DE OPERACIÓN: $estado_movi."),0,0,'L');
	$mipdf -> Ln (2);
	...
	$sql="SELECT CONCAT('E',LPAD(movimientos.id_movimiento,'6','0')) AS EGRESO, CONCAT(DATE_FORMAT(fecha_movi, '%d/%m/%Y'),' - ',hora_movi) AS REGISTRO, DATE_FORMAT(fecha_movi, '%d/%m/%Y') AS fecha_movi, TIME_FORMAT(hora_movi, '%r') AS hora_movi, operaciones.descripcion_ope, numero_movi, CONCAT(documentos.nombre_doc,' ',numero_movi) AS DOCUMENTO, descripcion_movi, categoria_ope, descripcion_ope, nombre_doc, tipo_doc, CONCAT('S/',FORMAT(monto_movi,2)) AS monto_movi, CONCAT(nombre,' ',apellido) AS nomape, nombre_suc, estado_movi FROM movimientos, operaciones, documentos, administrador, sucursales WHERE movimientos.id_sucursal=sucursales.id_sucursal AND movimientos.id=administrador.id AND movimientos.id_operacion=operaciones.id_operacion AND movimientos.id_documento=documentos.id_documento AND estado_movi='$estado_movi' AND tipo_ope='EGRESOS' AND movimientos.id_sucursal='$id_sucursal' ORDER BY REGISTRO DESC";
	...
	while ( $datos = $bd-> mostrar_registros($sql2))
	{
	    ...
	    $mipdf -> Cell(18,5,utf8_decode("$egreso"),1,0,'C');
