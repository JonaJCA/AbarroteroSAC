...
$sql2=$bd->consulta($sql);
	...
	$mipdf -> Setfont('Arial','B',12);
	$mipdf -> Ln (23);
	$mipdf -> Cell(190,10,"SOLICITUD DE $documento2 APROBADA",0,0,'C');
	$mipdf -> Ln (10);

	$mipdf -> Setfont('Arial','B',10);
	$mipdf -> Ln (10);	
 	$mipdf->MultiCell(190,7,"          MEDIANTE LA PRESENTE ES GRATO DIRIGIRME A USTED, $socio IDENTIFICADO CON DNI N.$dni Y REGISTRADO CON CODIGO $codigo A NOMBRE DE LA \"COOPERATIVA DE AHORRO Y CREDITO DE LOS TRABAJADORES DE LIMA SHERATON HOTEL\" Y A LA VEZ INFORMAR LA ACEPTACION RESPECTIVA PARA REALIZAR EL $documento2 CON UN MONTO INICIAL DE $monto FRACCIONADO EN $cuotas CUOTAS DE $fraccionado; EN LA CUAL DEPOSITAMOS NUESTRA CONFIANZA PARA AFIANZAR LOS VINCULOS ENTRE SOCIO Y COOPERATIVA, DE ACUERDO A LO QUE SE ESTABLECE EN LA NORMATIVA Y POLITICAS DE LA COOPERATIVA, ASI COMO SE DECRETA EN LA LEY N.15171 ARTICULO N.1 DECRETO LEY N.85 ARTICULO N.79.");
	...
	$mipdf -> Setfont('Arial','B',10);
	$mipdf -> Ln (3);
	$mipdf -> Cell(190,10,"JEFE DE COBRANZA",0,0,'C');
	$mipdf -> Ln (2);
	$mipdf -> Ln(5);
