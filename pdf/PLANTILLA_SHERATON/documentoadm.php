...
class MiPDF extends FPDF {
	...
	$cabeceraT = array("Codigo");
		...
		$mipdf -> Ln (2);
		$mipdf -> Cell(200,10,"Lista de documentos administrativos, $fecha",0,0,'C');
		$mipdf -> Ln (10);
...
$sql="SELECT CAST(@s:=@s+1 AS UNSIGNED) AS orden, nombre_doc, tipo_doc, CASE WHEN ruta IS NULL THEN 'NO' ELSE 'SI' END AS ruta FROM documento, (SELECT @s:=0) AS s WHERE tipo_doc = 'ADMINISTRATIVOS' ORDER BY tipo_doc, nombre_doc ASC;";
