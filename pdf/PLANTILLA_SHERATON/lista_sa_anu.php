...
class MiPDF extends FPDF {
	...
	$cabeceraT = array("Fecha");
		...
		$mipdf -> Ln (2);
		$mipdf -> Cell(200,10,"Lista de operaciones de saldo de aportes anuladas, $fecha",0,0,'C');
		$mipdf -> Ln (10);
...
$sql="SELECT CAST(@s:=@s+1 AS UNSIGNED) AS orden, DATE_FORMAT(fecha_movimiento, '%d/%m/%Y') AS fecha, socios.codigo, (CONCAT('S/',FORMAT(monto,2))) AS monto, case movimientos.operacion WHEN 'APORTE ANULADO' THEN 'APORTE' WHEN 'DEVOLUCION DE APORTE ANULADO' THEN 'DEVOLUCION' WHEN 'PAGARE ANULADO' THEN 'PAGARE' ELSE movimientos.operacion end operacion, CONCAT(documento.nombre_doc,'-',movimientos.numero_doc) AS Documento, CONCAT(administrador.nombre,' ',administrador.apellido) AS Encargado from movimientos, socios, documento, administrador, (SELECT @s:=0) AS s WHERE movimientos.operacion LIKE '%ANULADO' AND operacion NOT LIKE 'SOLICITU%' AND movimientos.id=administrador.id AND movimientos.id_socios=socios.id_socios AND movimientos.codigo_doc=documento.codigo_doc ORDER BY fecha_movimiento DESC;";
